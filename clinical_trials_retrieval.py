...
class ClinicalTrialsRetrieval:
    def __init__(self, db_path="DBs/trec_ct_2021_data.db", tfidf_matrix_path="joblibs/trec_ct_2021/trec_ct_2021_data_tfidf_matrix.joblib", vectorizer_path="joblibs/trec_ct_2021/trec_ct_2021_data_tfidf_vectorizer.joblib", inverted_index_path="joblibs/trec_ct_2021/trec_ct_2021_data_inverted_index.joblib"):
        self.conn = sqlite3.connect(db_path, check_same_thread=False)
        self.cursor = self.conn.cursor()
        self.dataset = ir_datasets.load("clinicaltrials/2021/trec-ct-2021")
        self.vectorizer = load(vectorizer_path)
        self.tfidf_matrix = load(tfidf_matrix_path)
        self.word2vec_model = load("joblibs/trec_ct_2021/trec_ct_2021_data_word2vec_model.joblib")
        self.doc_embeddings = load("joblibs/trec_ct_2021/trec_ct_2021_data_doc_embeddings.joblib")
        self.lda_model = LdaModel.load("joblibs/trec_ct_2021/lda_model_trec_ct_2021.model")
        self.lda_dict = Dictionary.load("joblibs/trec_ct_2021/lda_dictionary_trec_ct_2021.dict")
        self.doc_topics = load("joblibs/trec_ct_2021/trec_ct_2021_data_doc_topics.joblib")
        ...
        self.hybrid_doc_vectors = load("joblibs/trec_ct_2021/trec_ct_2021_data_hybrid_doc_vectors.joblib")

    def clean_text(self, text):
        # يزيل رموز التحكم غير القابلة للطباعة
        return re.sub(r'[\x00-\x1F\x7F]', '', text)

    def _print_test_queries(self, file_path="clinical_test_queries.txt"):
        limit = 10
        with open(file_path, "w", encoding="utf-8") as f:
            for index, query in enumerate(self.dataset.queries_iter()):
                ...
                query_text = self.clean_text(f"{query.text}")
                f.write(f"{query_id}\n{query_text}\n{'=' * 40}\n")

    def _load_documents(self):
        self.cursor.execute("SELECT doc_id, title, summary, description FROM documents")
        rows = self.cursor.fetchall()
        doc_ids, doc_texts = [], []
        for doc_id, title, summary, description in rows:
    ...
    def search(self, query: str, top_n=10):
        ...
        for i in top_local_indices:
            if similarities[i] > 0:
                ...
                self.cursor.execute(
                    "SELECT title, summary, description FROM documents WHERE doc_id = ?", (doc_id,)
                )
                row = self.cursor.fetchone()
                if row:
                    title, summary, description = row
